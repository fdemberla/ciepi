  ...
  try {
    ...
    if (!nombre || !email || !telefono || !comentarios) {
      return NextResponse.json(
        { error: "Datos incompletos. Nombre, email, teléfono y comentarios son obligatorios." },
        { status: 400 }
      );
    }
    ...
    if (recaptchaToken) {
      ...
      if (recaptchaScore !== null && recaptchaScore < 0.5) {
        return NextResponse.json(
          { error: "Verificación de seguridad fallida. Por favor, intenta de nuevo más tarde." },
          { status: 400 }
        );
      }
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Consulta enviada exitosamente. Nos pondremos en contacto contigo pronto.",
        data: {
          id: consultaId,
